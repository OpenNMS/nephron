...
public class KafkaInputTimestampPolicyFactoryTest {
    ...
    @Test
    public void testIt() {
        NephronOptions options = mock(NephronOptions.class);
        when(options.getDefaultMaxInputDelayMs()).thenReturn(TimeUnit.MINUTES.toMillis(2));

        TimestampPolicyFactory<String, FlowDocument> factory = getKafkaInputTimestampPolicyFactory(options);
        ...
        assertThat(
                policy.getWatermark(ctx), is(now.minus(Duration.standardSeconds(100)).minus(options.getDefaultMaxInputDelayMs())));
        ...
        assertThat(policy.getWatermark(ctx, now), is(now.minus(options.getDefaultMaxInputDelayMs())));
        ...
        assertThat(policy.getWatermark(ctx, now), is(backlogCheckTime.minus(options.getDefaultMaxInputDelayMs())));
    }
